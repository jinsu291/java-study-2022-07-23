...
public class Main20_2 {
    ...
    static class 전사A extends 사람 {
        ...
        void 공격2() {
            ...
            super.숨쉬다(); // 자신이 가지고 있는 `쉼쉬다` 메서드 중에서 전사A에서 직접 만든 `숨쉬다` 메서드를 제외한
                            // 것 중에서 가장 우선순위가 높은것을 사용
        }
    }

    static class 전사B extends 사람 {
        String 이름;
        // 전사B에는 이름속성이 있다.
        ...
        void 공격2() {
            System.out.println(super.이름 + "이(가) 공격!"); // 부모 클래스에서 받은 이름에는 아직 아무 값도 없다.
            super.숨쉬다(); // 자신이 가지고 있는 `쉼쉬다` 메서드 중에서 전사B에서 직접 만든 `숨쉬다` 메서드를 제외한 것
                            // 중에서 가장 우선순위가 높은것을 사용
        }
    }
}