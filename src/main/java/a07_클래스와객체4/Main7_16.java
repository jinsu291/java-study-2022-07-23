...
이 때 로봇오리 클래스가 오리를 상속받는 다면 `수영하다` 메서드를 오버라이드(메서드 재정의) 해야 합니다. 바로 이 때, 고무
오리 클래스에 있는 `수영하다` 메서드와 똑같은 `수영하다` 메서드가 로봇오리 클래스에도 생겨버리고 맙니다.

이 때 로봇오리 클래스가 고무오리를 상속받는 다면 `날다` 메서드를 오버라이드(메서드 재정의) 해야 합니다. 바로 이 때, 오리
 클래스에 있는 `날다` 메서드와 똑같은 `날다` 메서드가 로봇오리 클래스에도 생겨버리고 맙니다.

위 2가지 상황에서 똑같은 소스코드(메서드)가 여러 클래스에 존재하는 것을 중복라고 하는데, 상속만 가지고 중복을 제거하다
보면 단순한 문제는 쉽게 풀리지만 언젠가는 위와 같은 상황을 피할 수 없게 됩니다.
*/

public class Main7_16 {
